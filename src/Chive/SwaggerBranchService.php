<?php
/**
 * Class SwaggerBranchService
 * 作者: su
 * 时间: 2021/6/2 15:47
 * 备注: 分控制器生成对应的swagger文件
 */
...
class SwaggerBranchService
{
		...
		foreach ($servers as $server) {
			...
			array_walk_recursive($data, function ($item) use ($swagger, $routesService) {
				if ($item instanceof Handler && !($item->callback instanceof Closure)) {
					...
					if (!isset($this->swagger[$controller])) {
						$this->swagger[$controller] = $this->config->get('chive.swagger');
					}
					$this->initModel($controller);
					$swagger->addPath($controller, $action, $routesService);
				}
			});
			foreach ($this->swagger as $className => $s) {
				// 转换tags
				$this->swagger[$className]['tags'] = array_values($this->swagger[$className]['tags'] ?? []);
				$this->save($className, $s);
			}
		}
	}
	...
	public function addPath($className, $methodName, $routesService)
	{
		...
		$this->makeDefinition($definitionsAnno, $className);
		$definitionAnno && $this->makeDefinition([$definitionAnno], $className);

		// 加入tag
		$tag                                     = $classRoute->tag;
		$this->swagger[$className]['tags'][$tag] = [
			'name'        => $classRoute->tag,
		...
		$path                                               = $route['routeStr'];
		$method                                             = $route['method'];
		$this->swagger[$className]['paths'][$path][$method] = [
			'tags'        => [$tag],
	...
	public function makeDefinition($definitions, $className)
	{
		if (!$definitions) {
			return;
		}
		...
		foreach ($definitions as $definition) {
			...
			$this->swagger[$className]['definitions'][$defName]['properties'] = $formattedProps;
		}
	}
	...
	public function save($fileName, $s)
	{
		$fileName = str_replace("\\", "_", $fileName);
		$this->putFile($this->config->get('chive.swagger_output_path') . $fileName . '.json', json_encode($s, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
	}
	...
	private function initModel($className)
	{
		...
		$this->swagger[$className]['definitions']['ModelArray']  = $arraySchema;
		$this->swagger[$className]['definitions']['ModelObject'] = $objectSchema;
	}
