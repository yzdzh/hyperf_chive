...
//            throw new MongoDBException($e->getFile() . $e->getLine() . $e->getMessage());
     ...
     * @throws MongoDBException
     */
    public function update(string $namespace, array $filter = [], array $newObj = [], $options = []): bool
    {
        try {
            if (!empty($filter['_id']) && !($filter['_id'] instanceof ObjectId) && is_string($filter['_id']) ) {
                $filter['_id'] = new ObjectId($filter['_id']);
            }
            if (empty($options)) {
            ...
            $result        = $this->connection->executeBulkWrite($this->config['db'] . '.' . $namespace, $bulk, $written);
            $modifiedCount = $result->getModifiedCount();
            $update        = $modifiedCount ? true : false;
        } catch (\Exception $e) {
            $update = false;
//            throw new MongoDBException($e->getFile() . $e->getLine() . $e->getMessage());
        } finally {
     ...
     * @throws MongoDBException
     */
    public function updateReturnRow(string $namespace, array $filter = [], array $newObj = [], $options = []):int
    {
        try {
            if (!empty($filter['_id']) && !($filter['_id'] instanceof ObjectId) && is_string($filter['_id']) ) {
                $filter['_id'] = new ObjectId($filter['_id']);
