...
abstract class AbstractController
{
	 ...
	 * 成功返回
	 * @param array $data
	 * @return \Psr\Http\Message\ResponseInterface|Reply
	 */
	protected function success($data = [])
	{
		...
		if (Context::get(CommonConstants::IS_GRPC_KEY, false)) {
			$reply = new Reply();
			$reply->setCode($ret[ErrorHelper::RET_CODE]);
			$reply->setMsg($ret[ErrorHelper::RET_MSG]);
			$reply->setData(json_encode($ret[ErrorHelper::RET_DATA], JSON_UNESCAPED_UNICODE));
			$reply->setTotal($ret[ErrorHelper::RET_TOTAL]);
			return $reply;
		}
